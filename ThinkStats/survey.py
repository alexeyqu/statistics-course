...
class Table(object):
    ...
    def ConvertToDataFrame(self):
        """Convert records into pandas DataFrame format"""
        if len(self.records) == 0:
            return pd.DataFrame()
        columns = list(self.records[0].__dict__.keys())
        data = {col: [] for col in columns}
        for rec in self.records:
            for col in columns:
                data[col].append(getattr(rec, col))
...
def main(name, data_dir='.' + os.sep + 'data'):
    resp = Respondents()
    resp.ReadRecords(data_dir)
    print('Number of respondents: {0}'.format(len(resp.records)))

    preg = Pregnancies()
    preg.ReadRecords(data_dir)
    print('Number of pregnancies: {0}'.format(len(preg.records)))

    df = preg.ConvertToDataFrame()
